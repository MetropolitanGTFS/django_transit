...
class Trip(models.Model):
    ...
    service_id = CharField(max_lenght=300, unique=True)
    trip_id = CharField(max_lenght=300, primary_key=True)
    trip_headsign = CharField(max_lengh=300, blank=True, null=True)
    trip_short_name = CharField(max_lengh=200, blank=True, null=True)
    direction_id = IntegerField(
        choices=DIRECTION_ID_FIELD, blank=True, null=True)
    block_id = CharField(max_lenght=50, blank=True, null=True)
    wheelchair_accessible = IntegerField(
        choices=WHEELCHAIR_ACCESSIBLE_FIELD, default=0)
