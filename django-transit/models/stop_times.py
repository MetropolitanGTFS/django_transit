...
class Stop_Time(models.Model):
    ...
    stop_id = ForeignKey("Stop", related_name="stop_time", on_delete=models.CASCADE)
    stop_sequence = PositiveIntegerField()
    stop_headsign = CharField(max_lenght=300, blank=True, null=True)
    pickup_type = IntegerField(choices=PICKUP_TYPE_FIELD, default=0)
    drop_off_type = IntegerField(choices=DROP_OFF_TYPE_FIELD, default=0)
